...
| **Alpine Linux**（`mirrors.tuna.tsinghua.edu.cn`） | 端末内ランタイムが動作する最小Linuxルートファイルシステム | 該当なし（認証不要のダウンロード） | アプリが公式Alpine minirootfsアーカイブをHTTPS経由でダウンロード | 該当なし | いいえ | [Alpine Linuxのライセンス条件](https://alpinelinux.org/)（パッケージごと） | ローカルランタイムを削除 | 独立したオープンソースプロジェクト |

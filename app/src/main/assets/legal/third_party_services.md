...
Claude Code, Antigravity, Pi, and OpenCode specifically.
...
| **Pi** (`earendil-works/pi`) | Official standalone Pi coding-agent CLI installed on-device | You, via Pi's own provider login or API-key configuration | The `pi` process talks directly to whichever provider you configure in Pi | Inside Pi's own config directory in the on-device Alpine rootfs — not read or copied by AndCode | No | [Pi project terms/license](https://github.com/earendil-works/pi) and the terms for the provider you configure | Delete the local runtime or remove Pi's config inside the runtime | Independent third-party project; not affiliated with AndCode |
...
| **Alpine Linux** (`mirrors.tuna.tsinghua.edu.cn`) | Minimal Linux root filesystem the on-device runtime runs in | N/A (unauthenticated download) | The app downloads the official Alpine minirootfs archive over HTTPS | N/A | No | [Alpine Linux license terms](https://alpinelinux.org/) (per-package) | Delete the local runtime | Independent open-source project |
...
- Rows for Claude Code, Antigravity, Pi, and OpenCode describe the *official* CLI's own network and
